...
public String sendsms_fun(String content,String phone,String ip,int port,String un,String pw) {
	...
	try{
		...
		System.out.println("訊息長度:"+msg.length);
		if(msg.length>1206){
			byte[] msg1 = new byte[1205];
			System.arraycopy(msg, 0, msg1, 0, 1205);
			msg = msg1;
		}
		int t1=msg.length/smsv;  //商數
		int t2=msg.length%smsv;  //餘數
		if(t2!=0)t1++;  //有餘數+1
		byte dcs = new Byte((byte)0x08);
		byte udhi = new Byte((byte)1);
		System.out.println("訊息組合次數:"+t1+"次");
		//if(true)return;
		int cc = 1;
		if(msg.length > smsv+6){
			System.out.println("進入迴圈");
			while(msg.length > smsv){				
				byte[] by1 = new byte[smsv+6];
				by1[0] = 0x05; by1[1] = 0x00; by1[2] = 0x03; by1[3] = 0x4F; by1[4] = (byte)t1; by1[5] = (byte)cc;
				System.arraycopy(msg, 0, by1, 6, 134);
				//System.out.println(new String(by1, StandardCharsets.UTF_16BE));
				output = sns_Client1.submitMessage(un, pw, phone, by1, dcs, udhi);
				byte[] by2 = new byte[msg.length-smsv];
				System.arraycopy(msg, smsv, by2, 0, msg.length-smsv);
				msg = by2;
				cc++;
			}
			System.out.println(cc+"次");
			byte[] by1 = new byte[smsv+6];
			by1[0] = 0x05; by1[1] = 0x00; by1[2] = 0x03; by1[3] = 0x4F; by1[4] = (byte)t1; by1[5] = (byte)cc;
